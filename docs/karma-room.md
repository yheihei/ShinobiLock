...
`App/Resources/karma-lines.json` の `unlock_before_ad` が広告前の30種類、`stayed` が踏みとどまったときのセリフ。JSON内の文字列を編集してビルドすると反映される。改行は `\n` で指定する。
...
## 2026年9月15日：広告が表示できない場合

広告の読み込み・表示失敗時は `AdUnavailableView` で、既存の悲しい表情3種類と `ad_unavailable` のセリフ10種類を使います。表示の開始時に1組選び、同じ画面では変更しません。10秒後に実行ボタンが有効になりますが、自動実行はしません。「やめておく」は開始時から押せます。アプリを離れた場合も変更せず終了します。解除・休止・削除で共通の待機画面を使います。

シミュレータの専用確認アプリで、実際の `AdUnavailableView` と `RewardedAds` に広告SDKの失敗応答を与えて表示確認しました。iPhone SEサイズでカウント中の無効化、経過後の有効化、実行、中止を確認。accessibility3では両方のボタンが表示され、説明文を末尾までスクロールできました。この確認アプリは `build/fallback-preview/` にあり、実機のScreen Timeや広告配信の代わりにはなりません。

## 2026年9月17日：作者の広告収益に触れるセリフ4種

`unlock_before_ad` に4種類を追加し、合計30種類にした。作者への皮肉と、ユーザーが目標を達成できるという信頼を表す。文章は採用案を保ち、通常サイズで「素通り」「できる」などが途中で折り返されないように改行を調整した。

既存の「Shinobi SE QA」シミュレータ1台で、アプリ本体と同じ `KarmaRoomView`、文字スタイル、セリフJSONを使って表示を確認した。通常の文字サイズでは、4種類すべての全文と両方のボタンがスクロールせずに収まる。拡大表示の確認は途中までで、末尾までのスクロールは未確認。

表示画像は `output/karma-dialogue-20260917/line-1.png` から `line-4.png` に保存した。確認専用アプリのコンパイルとJSONの読み込みが成功し、追加した4種類は改行を除いて採用案と一致する。
